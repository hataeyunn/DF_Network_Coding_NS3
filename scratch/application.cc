...
int main(int argc, char *argv[]) {
	...
	for (int i = 0; i < activeCon; i++)
  {
		...
		sinkRight[i].Stop(Seconds(20000.));
		...
		if (i < nLN - 1)
     {
			...
			app[i]->Setup(socketLeft[i], sinkAddRight[i], 1040, 1000000000, DataRate("100Mbps"));
			leftnodes.Get(i)->AddApplication(app[i]);
			app[i]->SetStartTime(Seconds(1));
			app[i]->SetStopTime(Seconds(20000.));
		}
    ...
    else
    {
			...
			app[i - ( nLN - 1 )]->SetStopTime(Seconds(20000.));
		}
  ...
  app2->SetStopTime (Seconds (10000.));
  ...
  app3->SetStopTime (Seconds (10000.));
...
anim.EnableIpv4L3ProtocolCounters(Seconds(0), Seconds(20000));
//
